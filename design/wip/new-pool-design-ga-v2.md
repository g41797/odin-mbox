...
## 1. Core Philosophy
The Pool is a **mechanism-only** MPMC container for `PolyNode` carriers. It provides thread-safe reuse and lifecycle management while delegating all policy decisions (allocation limits, backpressure, and node sanitization) to a user-provided `FlowPolicy`.
...
3.  If `m^` is still valid, pushes to MPMC free-list and increments count.
...
for _ in 0..<100 {
    m: Maybe(^PolyNode)
    if pool_get(&my_pool, .Chunk, .Alloc_Only, &m) {
        pool_put(&my_pool, &m)
    }
}
```
...
```odin
m: Maybe(^PolyNode)
if pool_get(&pool, .Chunk, .Recycle_Or_Alloc, &m) {
